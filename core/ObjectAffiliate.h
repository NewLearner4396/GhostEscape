...
class ObjectAffiliate : public Object {
   protected:
    ObjectScreen* object_parent_ = nullptr;     // the object that this object is affiliated with
    ...
    Anchor anchor_ = Anchor::CENTER;              // the anchor point of the object
   ...
   public:
    ...
    void setSize(const glm::vec2& size) { this->size_ = size; setOffsetByAnchor(anchor_); }
    void setScale(float scale) { size_ *= scale; setOffsetByAnchor(anchor_); }
    ObjectScreen* getParent() const { return object_parent_; }
    void setParent(ObjectScreen* object_parent) { this->object_parent_ = object_parent; }
